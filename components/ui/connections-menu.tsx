    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute bottom-11 left-0 z-50 w-[300px] overflow-hidden rounded-2xl border border-border bg-card p-2 text-card-foreground shadow-2xl">
          <div className="space-y-0.5">
            {connectors.map(({ name, icon: Icon }) => (
              <button
                ...
                className="flex h-9 w-full items-center gap-3 rounded-xl px-2.5 text-left text-sm transition-colors hover:bg-muted"
              >
            ...
            <button
              ...
              className={cn(
                'flex h-9 w-full items-center gap-3 rounded-xl px-2.5 text-left text-sm transition-colors',
                browserEnabled ? 'bg-emerald-500/10' : 'hover:bg-muted'
              )}
            >
              <IconWorld className="size-4 text-muted-foreground" />
              <span className="flex-1">My Browser</span>
              <span className={cn('flex h-5 w-9 items-center rounded-full p-0.5 transition-colors', browserEnabled ? 'justify-end bg-blue-500' : 'justify-start bg-muted-foreground/30')}>
                <span className="size-4 rounded-full bg-white shadow-sm" />
              </span>
            </button>
          </div>
          <div className="my-2 border-t border-border" />
          <button type="button" onClick={() => setOpen(false)} className="flex h-9 w-full items-center gap-3 rounded-xl px-2.5 text-left text-sm hover:bg-muted">
          ...
          <button type="button" onClick={() => { onManage?.(); setOpen(false) }} className="flex h-9 w-full items-center gap-3 rounded-xl px-2.5 text-left text-sm hover:bg-muted">
