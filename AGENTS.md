...
- `docker compose up -d` - Run the application with Docker (includes PostgreSQL 17, Redis, Cohet AI app, and SearXNG)
- `docker compose down` - Stop all containers
- `docker compose down -v` - Stop all containers and remove volumes (deletes database data)
- `docker pull ghcr.io/miurla/cohet:latest` - Pull prebuilt Docker image
...
- Cohet AI Cloud deployments block `ENABLE_AUTH=false` automatically
...
- Guards against `COHET_CLOUD_DEPLOYMENT=true`
