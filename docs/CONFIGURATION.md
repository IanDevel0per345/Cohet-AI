...
This guide covers the optional features and their configuration in Cohet AI.
...
Cohet AI uses PostgreSQL for chat history storage. A database is **optional** for basic usage — without it, Cohet AI runs in a stateless mode where chat history is not persisted.
...
```bash
DATABASE_URL=postgresql://user:password@localhost:5432/cohet
```
...
Cohet AI dynamically detects available AI providers based on your API keys and displays a model selector in the UI. Set at least one provider API key to get started.
...
In **Cloud** mode (`COHET_CLOUD_DEPLOYMENT=true`), models are fixed by `config/models/cloud.json` and the model selector is not shown.
...
Cohet AI sends chat requests to `/v1/chat/completions`. You can provide either a base URL with or without `/v1`; both of these resolve to the same endpoint:
...
[OrcaRouter](https://www.orcarouter.ai) is an OpenAI-compatible endpoint that fronts 160+ models from OpenAI, Anthropic, Google, DeepSeek, xAI and others behind one API key. To use it with Cohet AI:
...
# Pin a few tool-capable models so Cohet AI's research agents work out of the box.
...
Cohet AI's research agents rely on tool calling, so make sure any model you select supports it. Restricting `OPENAI_COMPATIBLE_MODELS` to known tool-capable models (as above) is the simplest way to keep the selector clean and avoid hitting non-tool-capable upstreams.
...
By default, Cohet AI runs in **anonymous mode** (`ENABLE_AUTH=false`). This is ideal for personal, single-user environments.
...
Guest mode allows users to try Cohet AI without creating an account. Guest sessions are ephemeral — no chat history is stored.
...
Rate limiting only applies when `COHET_CLOUD_DEPLOYMENT=true`.
