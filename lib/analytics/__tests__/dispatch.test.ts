...
const original = process.env.COHET_CLOUD_DEPLOYMENT
...
describe('analytics dispatch', () => {
  ...
  afterEach(() => {
    process.env.COHET_CLOUD_DEPLOYMENT = original
  })

  it('is a no-op outside cloud deployment', async () => {
    process.env.COHET_CLOUD_DEPLOYMENT = 'false'
  ...
  it('forwards to the provider in cloud deployment', async () => {
    process.env.COHET_CLOUD_DEPLOYMENT = 'true'
