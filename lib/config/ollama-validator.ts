 ...
 * Checks which models support 'tools' capability required for Cohet AI
        ...
        if (invalidModels.length > 0) {
          console.error(
            '\n⚠️  ERROR: Configured Ollama models do not support tools!\n' +
              ...
              '\n\nCohet AI requires models with tools capability for web search functionality.\n' +
    ...
    if (validated.size === 0) {
      console.error(
        '\n⚠️  ERROR: No Ollama models with tools support found!\n' +
          ...
          '  ollama pull gpt-oss\n' +
          '  ollama pull deepseek-v3.1\n' +
          'Models without tools support will not work with Cohet AI.\n'
      )
    }
  } catch (error) {
    validationError = error as Error
    console.error('Ollama validation failed:', error)
    console.warn('Cohet AI will continue, but Ollama models may not work')
  }
}
...
export function validateOllamaModel(modelId: string): {
  ...
  if (!validatedModels!.has(modelId)) {
    return {
      valid: false,
      error: `Model "${modelId}" does not support tools capability required for Cohet AI. Please use a model with tools support.`
    }
  }
